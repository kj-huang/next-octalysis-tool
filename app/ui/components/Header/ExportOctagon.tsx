...
const ExportOctagon = ({ projectName, dimensions }: { projectName: string, dimensions : {width: number, height: number}}) => {
  const capturePng = async () => {
    const octalysisDom = document.querySelector('#octalysis-radar') as HTMLElement;
  ...
  return (
    <div style={{display: 'flex', alignItems: 'center'}}  onClick={capturePng}>
      ...
      <button>Export Octagon</button>
    </div>
  );
};

export default ExportOctagon;
